...
{
	SmBios *smbios = new SmBiosBinary("hp-dl380-g8.dmi");
	StructMap sysInfo = smbios->getNode(System);
	BOOST_REQUIRE_EQUAL(sysInfo["Manufacturer"], "HP");
	BOOST_REQUIRE_EQUAL(sysInfo["Product"], "ProLiant DL380p Gen8");
	BOOST_REQUIRE_EQUAL(sysInfo["Version"], "Not Specified");
	BOOST_REQUIRE_EQUAL(sysInfo["Serial"], "USE249NWXB");
	delete smbios;
}
